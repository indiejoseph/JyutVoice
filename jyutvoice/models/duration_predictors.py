...
from jyutvoice.flow.decoder import SinusoidalPosEmb, TimestepEmbedding
from jyutvoice.models.text_encoder import LayerNorm
...
class DeterministicDurationPredictor(nn.Module):
    ...
    def compute_loss(self, durations, enc_outputs, x_mask):
        return F.mse_loss(
            self.estimator(enc_outputs, x_mask), durations, reduction="sum"
        ) / torch.sum(x_mask)
...
class FlowMatchingDurationPrediction(nn.Module):
    ...
    def forward(self, enc_outputs, mask, n_timesteps=500, temperature=1):
        ...
        z = (
            torch.randn((b, 1, t), device=enc_outputs.device, dtype=enc_outputs.dtype)
            * temperature
        )
    ...
    def compute_loss(self, x1, enc_outputs, mask):
        ...
        loss = F.mse_loss(
            self.estimator(y, mask, enc_outputs, t.squeeze()), u, reduction="sum"
