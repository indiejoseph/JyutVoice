...
def penalize_logits(logits, decoded_tokens, eos_token_id=None, penalty=1.2):
    """
    Apply repetition penalty to logits based on previous decoded tokens.

    Args:
        logits (Tensor): [vocab_size], raw logits for current step.
        decoded_tokens (List[int]): previously generated token IDs.
        eos_token_id (int): EOS token ID to exclude from penalty.
        penalty (float): penalty factor (>1.0 means penalize more).
    Returns:
        Tensor: modified logits.
    """
...
def repeat_cache(cache, K: int):
    """
    Repeat each tensor in attention/CNN cache K times along batch dimension.
    """
    return [x.repeat(K, *[1] * (x.ndim - 1)) for x in cache]
...
def convert_onnx_to_trt(trt_model, trt_kwargs, onnx_model, fp16):
    ...
    print("Succesfully convert onnx to trt...")
