...
from jyutvoice.transformer.convolution import ConvolutionModule
from jyutvoice.transformer.encoder_layer import ConformerEncoderLayer
from jyutvoice.transformer.positionwise_feed_forward import PositionwiseFeedForward
...
class Upsample1D(nn.Module):
    ...
    def forward(
        ...
        conv_cache: torch.Tensor = torch.zeros(0, 0, 0),
...
class PreLookaheadLayer(nn.Module):
    ...
    def forward(
        ...
        conv2_cache: torch.Tensor = torch.zeros(0, 0, 0),
...
class UpsampleConformerEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.up_embed = SUBSAMPLE_CLASSES[input_layer](
            input_size,
            output_size,
            dropout_rate,
            EMB_CLASSES[pos_enc_layer_type](output_size, positional_dropout_rate),
    ...
    def forward_chunk(
        ...
        tmp_masks = torch.ones(
            1, context.size(1), device=context.device, dtype=torch.bool
        )
        tmp_masks = tmp_masks.unsqueeze(1)
        if context.size(1) != 0:
        ...
        xs, pre_lookahead_layer_conv2_cache = self.pre_lookahead_layer(
            xs, context, pre_lookahead_layer_conv2_cache
        )
        # NOTE in cache mode we do not need to call add_optional_chunk_mask
        chunk_masks = torch.ones(
            (1, xs.size(1), offset), dtype=torch.bool, device=xs.device
        )
        ...
        for index, layer in enumerate(self.encoders):
            xs, chunk_masks, encoders_kv_cache_new, _ = layer(
                xs, chunk_masks, pos_emb, mask_pad, encoders_kv_cache[index]
            )
        ...
        # upsample
        xs = xs.transpose(1, 2).contiguous()
        xs, xs_lens, upsample_conv_cache = self.up_layer(
            xs, xs_lens, upsample_conv_cache
        )
        xs = xs.transpose(1, 2).contiguous()
        ...
        # conformer encoder
        chunk_masks = torch.ones(
            (1, xs.size(1), upsample_offset), dtype=torch.bool, device=xs.device
        )
        mask_pad = torch.ones((0, 0, 0), dtype=torch.bool, device=xs.device)
        upsample_kv_cache_list = []
        for index, layer in enumerate(self.up_encoders):
            xs, chunk_masks, upsample_kv_cache_new, _ = layer(
                xs, chunk_masks, pos_emb, mask_pad, upsample_kv_cache[index]
            )
